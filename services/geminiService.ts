        ...
        recipes: {
            ...
            items: {
                ...
                properties: {
                    ...
                    estimatedCost: { type: Type.STRING, description: "Примерная стоимость блюда (например, '300 руб.')" },
                    nutrition: {
                        type: Type.OBJECT,
                        properties: {
                    ...
                    ingredients: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                amount: { type: Type.STRING, description: "Количество (например, '200 г' или '1 шт.')" },
                            },
                ...
                required: ["recipeName", "description", "cookingTime", "estimatedCost", "nutrition", "ingredients", "instructions", "kitchenware"]
            }
        }
    },
    required: ["recipes"]
...
export async function generateRecipeAndImage(request: RecipeRequest): Promise<Recipe[]> {
    try {
        ...
        if (!recipeDataArray || !Array.isArray(recipeDataArray) || recipeDataArray.length === 0) {
            throw new Error("AI не смог придумать рецепты из этих ингредиентов. Попробуйте изменить состав или параметры.");
        }

        const fullRecipes = await Promise.all(
            recipeDataArray.map(async (recipeData: Omit<Recipe, 'imageUrl' | 'ingredients'> & { ingredients: { name: string, amount: string }[] }) => {
                
                const availableIngredientsLower = request.ingredients.map(i => i.toLowerCase());
                const processedIngredients: Ingredient[] = recipeData.ingredients.map((ing: { name: string; amount: string; }) => {
                    const hasIngredient = availableIngredientsLower.some(availIng => ing.name.toLowerCase().includes(availIng) || availIng.includes(ing.name.toLowerCase()));
        ...
        if (error instanceof Error) {
            if (error.message.startsWith("AI не смог") || error.message.startsWith("AI-шеф вернул") || error.message.startsWith("Не удалось создать изображение")) {
                throw error;
            }
        }
        
        throw new Error("Не удалось создать рецепты. AI-шеф может быть занят. Пожалуйста, попробуйте позже.");
    }
}