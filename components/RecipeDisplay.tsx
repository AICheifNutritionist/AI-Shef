...
export const RecipeDisplay: React.FC<RecipeDisplayProps> = ({ recipe }) => {
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-xl border border-gray-200">
      <img src={recipe.photo} alt={recipe.recipeName} className="w-full h-64 object-cover" />
          ...
          <div>
            ...
            <ul className="space-y-2">
              {recipe.ingredients.map((ing, index) => (
                <li
                  key={index}
                  className={`flex items-start gap-3 p-2 rounded-md ${ing.status === 'have' ? 'bg-blue-50' : 'bg-yellow-50'}`}
                >
                  {ing.status === 'have' ? (
            ...
            <ol className="space-y-4">
              {recipe.instructions.map((step, index) => (
                <li key={index} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-8 h-8 bg-green-600 text-white font-bold rounded-full flex items-center justify-center">
                    {index + 1}
                  </span>
