    ...
    <div className="bg-gray-100 p-6 rounded-lg space-y-4 border border-gray-200">
      <div>
        <div className="flex items-center justify-between">
            ...
            >
                <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${willingToShop ? 'translate-x-6' : 'translate-x-1'}`} />
            </div>
            <input id="willing-to-shop" type="checkbox" className="sr-only" checked={willingToShop} onChange={() => setWillingToShop(!willingToShop)} />
        </div>
         <p className="text-sm text-gray-500 mt-1">AI-шеф сможет предложить более интересные рецепты, если ему можно добавить 1-2 новых ингредиента.</p>
      </div>

      {willingToShop && (
        <div className="animate-fade-in">
          <label htmlFor="shopping-budget" className="block text-lg font-semibold text-gray-700 mb-2">
            Макс. бюджет на докупку: <span className="text-green-600 font-bold">{shoppingBudget} руб.</span>
          </label>
          <input
            id="shopping-budget"
            type="range"
